...
class BaseBootstrapper(object):
    ...
    def ensure_rust_targets(self, rustup):
        ...
        targets = [line.split()[0] for line in target_list.split('\n')
                if 'installed' in line or 'default' in line]
        print('Rust supports %s targets.' % ', '.join(targets))

        # Provide 32-bit support if we're on Windows.
        win32 = 'i686-pc-windows-msvc'
        if rust.platform() == 'x86_64-pc-windows-msvc' and
           not win32 in targets:
